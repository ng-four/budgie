...
describe("endpoints should be protected", function() {
  describe('GET', function() {
    it("should return 401 when no user logged in and getting expenses",
      function(done) {
        request
          .get('/expenses/7')
  ...
  describe('POST', function() {
    it("should return 401 when no user loggin in and posting an expense",
      function(done) {
        request
          .post('/expenses')
          .send(querystring.stringify({
            ...
            'amount': '10',
            'category': 'household',
            'spent_date': '2016-02-09 12:02:10'
          }))
          .expect(401, function(err) {
            if (err) {
...
describe("goals", function() {
        ...
        .expect(200, function(err, res) {
          ...
          request
            ...
            .send(querystring.stringify({
              ...
              'notes': 'Testarossa - black, preferably'
            }))
            .expect(201, function(err) {
              if (err) {
  ...
  describe('PUT', function(token, done) {
    it("should add money to a new goal", function(done) {
      request
        .post('/signup')
        .send(querystring.stringify({
          'email': 'test2@test2.com',
          ...
          request
            ...
            .expect(201, function(err, res) {
              if (err) {
                ...
                .expect(201, function(err, res) {
                  if (err) {
                    console.log('error adding money to a goal', err);
                  }
                  done(err);
                });
              }
