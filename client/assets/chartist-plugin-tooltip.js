                  ...
                  if (options.tooltipFnc) {
                      ...
                      if (meta) {
                          ...
                          if (chart instanceof Chartist.Pie) {
                              // console.log("this is $point on line 95 in chartist-plugin", $point);
                              var grandparent = $point.parentNode.parentNode;
                              // console.log("this is the grandparent", grandparent);
                              for(var i = 0; i<grandparent.childNodes.length; i++){
                                if($point === grandparent.childNodes[i].childNodes[0]){
                                  var index = i;
                                  break;
                                }
                              }

                              var label = grandparent.lastChild.childNodes[(grandparent.lastChild.childNodes.length - 1) - index].innerHTML;

                              // var label = next($point, 'ct-label');
                              if (label.length > 0) {
                                  console.log("this is label on line 111", label);
                                  console.log("this is label.length on line 112", label.length);
